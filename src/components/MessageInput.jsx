...
export default function MessageInput({ inputRef, value, onChange, onSubmit, onFileSelect, imagePreview, onCancelImage, uploading, uploadErr, peerName }) {
  ...
  return (
    <div style={{ background:'var(--bg-panel)', borderTop: imagePreview ? 'none' : '1px solid var(--border)', flexShrink:0 }}>
      {imagePreview && (
      ...
      <div style={{ padding:'12px 18px' }}>
        ...
        <form onSubmit={onSubmit} style={{ display:'flex', gap:8, alignItems:'center' }}>
          <button type="button" onClick={() => fileRef.current?.click()}
            style={{ width:42, height:42, borderRadius:'var(--radius-sm)', flexShrink:0, background:'rgba(255,255,255,0.04)', border:'1px solid var(--border)', color:'var(--text-muted)', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }}
